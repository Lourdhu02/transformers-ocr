...
def train_one_epoch(model, loader, optimizer, criterion, scaler,
    ...
    for step, (imgs, targets, lengths, _) in enumerate(pbar, 1):
        ...
        imgs, targets, lengths = cutmix_batch(
            imgs, targets, lengths, alpha=cutmix_alpha
        )
...
def print_header():
    cols = (
        f"{'Ep':>4} | {'Train':>8} | {'Val':>8} | "
        f"{'Exact%':>7} | {'Char%':>7} | {'LR':>9} | "
        f"{'Time':>6} | {'Status'}"
    )
    print("-" * len(cols))
    print(cols)
...
def main():
    ...
    if args.weights and os.path.isfile(args.weights):
        ...
        print(
            f"  resumed from {args.weights}  epoch={ckpt['epoch']}  "
