...
def load_model(weights_path, variant, device):
    ckpt    = torch.load(weights_path, map_location=device)
    cfg     = ckpt.get("cfg", CONFIGS[variant])
    encoder = Encoder(cfg["chars"])
    model   = build_model(variant, cfg["img_h"], cfg["img_w"],
                          encoder.num_classes,
...
def predict_single(model, encoder, img_bgr, cfg, device, tf,
                   use_tta=False):
    ...
    if not use_tta:
        ...
        pred, conf = encoder.decode_beam(lp_np,
    ...
    votes = []
    bcs   = [(-0.3, -0.3), (-0.15, 0.0), (0.0, 0.0),
              (0.15, 0.15), (0.3, 0.3)]
    for b, c in bcs:
        aug  = A.Compose([
            A.RandomBrightnessContrast(brightness_limit=(b, b),
        ...
        with torch.no_grad(), autocast():
            log_probs = model(tensor)
        lp_np    = log_probs[0].cpu().numpy()
        pred, sc = encoder.decode_beam(lp_np,
                                        beam_width=cfg.get("beam_width", 10),
                                        lexicon_pattern=cfg.get("lexicon_re"))
